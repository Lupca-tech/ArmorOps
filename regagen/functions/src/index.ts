...
// --- TYPE DEFINITIONS (Copied from your app's types.ts) ---
type EditablePlatform = "web" | "facebook" | "linkedin" | "x" | "tiktok" | "youtube";
...
const allPlatformProperties = {
  ...
  facebook: {
    type: Type.OBJECT,
    properties: {postText: {type: Type.STRING, description: "A short, engaging Facebook post with emojis and a call to action."}},
    required: ["postText"],
  },
  linkedin: {
    type: Type.OBJECT,
    properties: {postText: {type: Type.STRING, description: "A professional, business-oriented LinkedIn post. It should be insightful, use relevant hashtags, and encourage professional discussion."}},
    required: ["postText"],
  },
  x: {
    type: Type.OBJECT,
    properties: {postText: {type: Type.STRING, description: "A concise, impactful post for X (formerly Twitter), under 280 characters, with relevant hashtags."}},
    required: ["postText"],
  },
  tiktok: {
    type: Type.OBJECT,
    properties: {script: {type: Type.STRING, description: "A script for a 30-60 second TikTok/YT Shorts video, including visual cues and spoken lines."}},
    required: ["script"],
  },
  youtube: {
    type: Type.OBJECT,
...
export const generateContent = functions.https.onCall(async (data, context) => {
  ...
  const {topic, language, shouldGenerateImage, selectedPlatforms, brandVoiceProfile} = data as {
    topic: string;
    language: string;
 ...
 */
export const regeneratePlatform = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
  }
 ...
 */
export const analyzeBrandVoice = functions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError("unauthenticated", "The function must be called while authenticated.");
  }
  ...
  try {
    const schema = {
      type: Type.OBJECT,
      properties: {
        toneAndManner: {type: Type.STRING, description: "A concise description of the overall tone and manner."},
        vocabularyLevel: {type: Type.STRING, description: "The level of vocabulary used."},
        sentenceStructure: {type: Type.STRING, description: "Describe the typical sentence structure."},
        dos: {
          type: Type.ARRAY,
